...
AWS_ASSUMED_ROLE_ARN_REGEX = compile(r'arn:(aws[a-zA-Z-]*)?:sts::\d{12}:assumed-role\/([A-Za-z0-9+=,.@_\-\/]+)\/([A-Za-z0-9+=,.@_\-]+)')
...
AWS_ACCESS_KEY_REGEX = compile(r'(?:A3T[A-Z0-9]|AKIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16}')
AWS_SECRET_KEY_REGEX = compile(r'[0-9a-zA-Z\\/+]{40}')
AWS_SESSION_TOKEN_REGEX = compile(r'[0-9a-zA-Z\\/+]{364}')
AWS_ACCOUNT_ID_REGEX = compile(r'[0-9]{12}')

##### Microsoft Related Regex Patterns #####
UUID_LOWERCASE_REGEX = compile(r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}')
