...
class Module(AWSModule):
    ...
    def __init__(self, framework) -> None:
        ...
        self._options.add_integer(Module.OPT_MAX_QUERIES, 'Maximum number of queries to que simultaneously (max: 30)', True, 5)
        self._options.add_integer(Module.OPT_POLL_DELAY, 'Number of seconds to wait before re-checking query status (1 - 300)', True, 5)
        self._options.add_integer(Module.OPT_RECORD_LIMIT, 'Limit to impose on the number of log records returned by the queries', True, 1000)
        ...
        self._options.add_string(Module.OPT_LOG_GROUPS, 'Names of log group(s) to include within the query (max 50). (F/P)', True)
        self._options.add_string(Module.OPT_SEARCH_PATTERN, 'Regualr expression to use as the search term within log messages (F/P)')
    ...
    def validate_options(self) -> tuple:
        ...
        lg_len = len(self.get_opt_multiline(Module.OPT_LOG_GROUPS, unique=True))
    ...
    def perform_and_monitor_query(self, pattern: str, start: int, end: int, query_num: int) -> bool:
        ...
        delay = self.get_opt(Module.OPT_POLL_DELAY)
        log_groups = self.get_opt_multiline(Module.OPT_LOG_GROUPS)
    ...
    def run(self):
        ...
        search_pattern = self.get_opt_multiline(Module.OPT_SEARCH_PATTERN)
