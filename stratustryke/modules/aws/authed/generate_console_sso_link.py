...
class Module(AWSModule):
    def __init__(self, framework) -> None:
        ...
        self._options.add_integer('DURATION', 'Duration in minutes for the console session to be valid for', True, 60)
        self._options.add_string('SESSION_NAME', 'Name for federated user session if using long-term credentials', False, 'stratustryke', '^[a-zA-Z\_\=,\.\@-]*$')
    ...
    def run(self):
        cred = self.get_cred()
        duration = self.get_opt('DURATION') * 60 # duration is sent in seconds to the APIs
        policy = str({
            "Version": "2012-10-17",
            "Statement": [
                {
