...
class Module(AWSModule):
    def __init__(self, framework) -> None:
        ...
        self._options.add_string('LAMBDA_PREFIX', 'Prefix for lambdas to include (default: match ALL functions)', False)
        self._options.add_boolean('ALL_VERSIONS', '(WORK IN PROGRESS) When enabled, attemmpts to insepect ALL unique function versions in history.')
        self._options.add_string('DOWNLOAD_DIR', 'Directory files will be downloaded to', True, module_data_dir(self.name))
        self._options.add_boolean('VERBOSE', 'When enabled, prints lambda function tags and environment variables to console', True, True)
        self._options.add_boolean('NO_DOWNLOAD', 'When enabled, disables source code download and only enumerates configs', True, False)
    ...
    def list_lambdas(self) -> list:
        ...
        prefix = self.get_opt('LAMBDA_PREFIX')
    ...
    def download_source(self, name: str) -> bool:
        ...
        if self.get_opt('NO_DOWNLOAD'): return True
