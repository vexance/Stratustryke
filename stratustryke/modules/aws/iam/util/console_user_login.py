...
class Module(StratustrykeModule):
    ...
    def __init__(self, framework) -> None:
        ...
        self._options.add_string(Module.OPT_TARGET_ACCOUNT_ID, 'Target AWS account to authenticate against', True, regex='^[0-9]{12}$')
        self._options.add_string(Module.OPT_USERNAME, 'Username supplied in the login attempt (Supports file: prefix)', True)
        self._options.add_string(Module.OPT_PASSWORD, 'Password to use for attempted authentication.', True, sensitive=True)
        self._options.add_integer(Module.OPT_TIME_DELAY, 'Time in seconds to wait between requests', False, 0)
    ...
    def validate_options(self) -> tuple:
        ...
        user = self.get_opt(Module.OPT_USERNAME)
    ...
    def attempt_login(self, user: str, pwd: str) -> tuple:
        ...
        acc_id = self.get_opt(Module.OPT_TARGET_ACCOUNT_ID)
        session = Session()
    ...
    def password_spray(self, users: list, pwd: str) -> list:
        ...
        delay = self.get_opt(Module.OPT_TIME_DELAY)
    ...
    def run(self):
        uname = self.get_opt(Module.OPT_USERNAME)
        passwd = self.get_opt(Module.OPT_PASSWORD)

        is_pasted = self._options.get_opt(Module.OPT_USERNAME)._pasted
