...
class Module(AWSModule):
    ...
    OPT_DEST_REGION = 'DEST_REGION'
    OPT_DESCRIPTION = 'DESCRIPTION'
    OPT_OUTFILE = 'OUT_FILE'
    ...
    def __init__(self, framework) -> None:
        ...
        self._options.add_string(Module.OPT_SNAPSHOT_ID, 'Id of the snapshot to obtain a copy of', True)
        self._options.get_opt(Module.OPT_AWS_REGION)._desc = 'Region containing the target snapshot'
        self._options.add_string(Module.OPT_DEST_REGION, 'Destination region to copy the snapshot to', True, self.get_opt(Module.OPT_AWS_REGION))
        self._options.add_string(Module.OPT_DESCRIPTION, 'Description to apply to the new snapshot copy', False, 'Stratustryke')
        self._options.add_string(Module.OPT_OUTFILE, 'Name of output file to copy the snapshot as', False)
        self._options.add_string(Module.OPT_DOWNLOAD_DIR, 'Directory to save the snapshot copy to', False, module_data_dir(self.name))
    ...
    def run(self):
        snap_id = self.get_opt(Module.OPT_SNAPSHOT_ID)
        src_region = self.get_opt(Module.OPT_AWS_REGION)
        dest_region = self.get_opt(Module.OPT_DEST_REGION)
        desc = self.get_opt(Module.OPT_DESCRIPTION)
        download_dir = self.get_opt(Module.OPT_DOWNLOAD_DIR)
        outfile = self.get_opt(Module.OPT_OUTFILE)
