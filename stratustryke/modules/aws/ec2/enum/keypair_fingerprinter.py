...
class Module(AWSModule):
    ...
    def load_private_key_from_pem(self, pem_text: str, key_pwd: str):
        ...
        try:
            if 'BEGIN OPENSSH PRIVATE KEY' in pem_text: # OpenSSH format
                return serialization.load_ssh_private_key(pem_text.encode('utf-8'), password=key_pwd)
    ...
    def openssh_sha256_fingerprint(self, key) -> str:
        ...
        return base64.b64encode(digest).decode('ascii')  # ssh-keygen style (no padding)
    ...
    def parse_provided_keys(self) -> list[str]:
        ...
        if opt == None: opt = [self.get_opt(Module.OPT_PRIVATE_KEY)]
        ...
        if len(opt) == 1: # Either a file or a directory
            ...
            for keyfile in key_paths:
                try:
                    ...
                    imported_key = self.load_private_key_from_pem(keytext, password)
                    if imported_key == None:
                        self.print_failure(f'Unable to load keytext in {keyfile}')
                        continue

                    ret.append(imported_key)
    ...
    def calculate_fingerprints(self, key) -> list:
    ...
    def get_regional_keypair_fingerprints(self, region: str, accumulator: dict) -> int:
        ...
        try:
            ...
            if len(pairs) > 0:
                ...
                self.print_warning(f'No key pairs found in region {region}')
    ...
    def match_fingerprints(self, key_fingerprints: list[str], deployed_keypairs: dict) -> dict:
    ...
    def run(self) -> None:
        ...
        for i, key_text in enumerate(keys):
            try:
                key_fingerprints.extend(self.calculate_fingerprints(key_text))
