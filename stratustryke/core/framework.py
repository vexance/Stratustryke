...
class StratustrykeFramework(object):
    ...
    def __init__(self, stdout = None):
        ...
        self._config.add_string('HTTP_PROXY', 'Proxy (schema://host:port) for modules to use as an HTTP/S proxy for web traffic', True, None, '(http[s]?|socks[45][h]?)[:]\/\/.*[:][0-9]{1,5}')
        self._config.add_boolean('HTTP_VERIFTY_SSL', 'Enable or disable SSL/TLS verification when modules perform manual HTTP requests', True, stratustryke.settings.HTTP_VERIFY_SSL)
